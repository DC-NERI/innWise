...
import { Loader2, PlusCircle, Users as UsersIconAliased, Edit, Trash2, ArchiveRestore, KeyRound } from 'lucide-react';
...
import { resetUserPasswordAdmin } from '@/actions/admin/users/resetUserPasswordAdmin';
...
export default function UsersContent({ tenantId, adminUserId }: UsersContentProps) {
  ...
  const [activeUserManagementTab, setActiveUserManagementTab] = useState("active");
  ...
  const filteredUsers = users.filter(user => user.status === (activeUserManagementTab === "active" ? HOTEL_ENTITY_STATUS.ACTIVE : HOTEL_ENTITY_STATUS.ARCHIVED));
  ...
  const renderUserFormFields = () => {
    ...
    return (
      <React.Fragment>
        <FormField control={userForm.control} name="first_name" render={({ field }) => {
          return (
        ...
        <FormField control={userForm.control} name="last_name" render={({ field }) => {
          return (
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <div className="flex items-center space-x-2">
            <UsersIconAliased className="h-6 w-6 text-primary" />
      ...
      <CardContent>
        <Tabs value={activeUserManagementTab} onValueChange={setActiveUserManagementTab}>
