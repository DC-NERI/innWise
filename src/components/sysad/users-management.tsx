...
export default function UsersManagement() {
  ...
  const form = useForm<UserFormValues>({
    ...
    defaultValues: isEditing && selectedUser ? {
  ...
  useEffect(() => {
    const newResolver = zodResolver(isEditing ? userUpdateSchemaSysAd : userCreateSchema);
    ...
    form.reset(newDefaults, { resolver: newResolver } as any); // Use 'as any' if types conflict during resolver swap
  }, [isEditing, selectedUser, form.reset, toast]);
  ...
  const handleAddSubmit = async (data: UserCreateData) => {
    ...
    } catch (e) { toast({ title: "Error", description: "Unexpected error during user creation.", variant: "destructive" }); }
  ...
  const handleEditSubmit = async (data: UserUpdateDataSysAd) => {
    ...
    } catch (e) { toast({ title: "Error", description: "Unexpected error during user update.", variant: "destructive" }); }
    finally { setIsSubmitting(false); }
  };
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Dialog 
            key={isEditing ? `edit-${selectedUser?.id}` : 'add'} // Add key to force re-mount of dialog
            open={isAddDialogOpen || isEditDialogOpen} 
            onOpenChange={(open) => {
                if (!open) {
                    setIsAddDialogOpen(false);
                    setIsEditDialogOpen(false);
                    setSelectedUser(null); // This will trigger the useEffect to reset form for 'add' mode
                } else {
                    // Logic to open is handled by DialogTrigger or direct state set
                }
            }}
        >
          <DialogTrigger asChild>
            <Button onClick={() => {
              setSelectedUser(null); // Ensure isEditing is false
              ...
              <TableBody>{filteredUsers.map(u => (
                <TableRow key={u.id}>
                  ...
                  <TableCell className="text-right space-x-2">
                    <Button variant="outline" size="sm" onClick={() => { setSelectedUser(u); setIsEditDialogOpen(true);}}><Edit className="mr-1 h-3 w-3" /> Edit</Button>
