...
export async function assignRoomAndCheckIn(
  ...
  staffUserId: number,
  tenantId: number,
  branchId: number // This should be the branch ID of the staff member performing the action
): Promise<{ success: boolean; message?: string; updatedRoomData?: Partial<HotelRoom> & { id: number }, updatedTransaction?: Transaction }> {
  console.log(`[assignRoomAndCheckIn] Action started. TxID: ${transactionId}, RoomID: ${roomId}, StaffID: ${staffUserId}, TenantID: ${tenantId}, BranchID (Staff's Branch): ${branchId}`);

  // Critical constants check
  const EXPECTED_RESERVATION_STATUS = TRANSACTION_LIFECYCLE_STATUS.RESERVATION_NO_ROOM; // Should be 3
  const EXPECTED_IS_ACCEPTED_STATUS = TRANSACTION_IS_ACCEPTED_STATUS.ACCEPTED; // Should be 2
  const NEW_TRANSACTION_LIFECYCLE_STATUS = TRANSACTION_LIFECYCLE_STATUS.CHECKED_IN; // Should be 0
  const ROOM_NOW_OCCUPIED_STATUS = ROOM_AVAILABILITY_STATUS.OCCUPIED; // Should be 1
  const ROOM_DEFINITION_ACTIVE_STATUS = HOTEL_ENTITY_STATUS.ACTIVE; // Should be '1'
  ...
  try {
    ...
    // Pre-check the reservation to ensure it's assignable
    const SELECT_RESERVATION_SQL = `
      SELECT status, is_accepted, client_name, hotel_rate_id, reserved_check_in_datetime, is_paid
      FROM transactions
      WHERE id = $1 AND tenant_id = $2 AND branch_id = $3 -- Ensure this branchId is the reservation's actual target branch
        AND hotel_room_id IS NULL
        AND status::INTEGER = $4 -- Expected: RESERVATION_NO_ROOM (3)
        AND is_accepted = $5     -- Expected: ACCEPTED (2)
    ...
    const reservationRes = await client.query(SELECT_RESERVATION_SQL, [
      transactionId,
      tenantId, // This tenantId is from the staff's context.
      branchId, // This branchId is from the staff's context. The transaction itself must belong to this branch.
      EXPECTED_RESERVATION_STATUS,
      EXPECTED_IS_ACCEPTED_STATUS
    ]);
    ...
    if (reservationRes.rows.length === 0) {
      ...
      // For debugging, fetch the transaction without status filters to see its current state
      ...
      return { success: false, message: `Reservation (ID: ${transactionId}) not found, already assigned a room, or not in a valid state for room assignment.` };
    }
    const reservation = reservationRes.rows[0];
    console.log(`[assignRoomAndCheckIn] Found reservation to assign:`, reservation);

    // Check if room is actually available and active
    const ROOM_CHECK_SQL = `
      SELECT is_available, cleaning_status, room_name
      FROM hotel_room
      WHERE id = $1 AND tenant_id = $2 AND branch_id = $3 AND status = $4
      FOR UPDATE; -- Lock the room row
    `;
    ...
    const UPDATE_TRANSACTION_SQL = `
      ...
      RETURNING *, check_in_time as effective_check_in_time; -- Return effective_check_in_time
    `;
    ...
    checkInTimeForReturn = updatedTransactionRow.effective_check_in_time; // Use the time set by DB
    console.log(`[assignRoomAndCheckIn] Transaction ${transactionId} updated:`, updatedTransactionRow);


    const UPDATE_ROOM_SQL = `
      UPDATE hotel_room
      SET is_available = $1, -- OCCUPIED (1)
          transaction_id = $2,
          updated_at = (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Manila')
    ...
    const staffUserRes = await client.query('SELECT username FROM users WHERE id = $1', [staffUserId]);
    const staffUsername = staffUserRes.rows.length > 0 ? staffUserRes.rows[0].username : `User (ID: ${staffUserId})`;
    ...
    await logActivity({
      ...
      actor_username: staffUsername,
      action_type: 'STAFF_ASSIGNED_ROOM_AND_CHECKED_IN',
      description: `Staff '${staffUsername}' assigned room '${updatedRoomDbRow.room_name}' (${updatedRoomDbRow.room_code}) to client '${updatedTransactionRow.client_name}' (Tx ID: ${transactionId}) and checked them in. Rate: ${rateName}.`,
    ...
    // Prepare data for client-side update
    const updatedRoomData: Partial<HotelRoom> & { id: number } = {
      ...
      active_transaction_client_name: updatedTransactionRow.client_name,
      active_transaction_check_in_time: checkInTimeForReturn,
      active_transaction_rate_name: rateName,
      active_transaction_rate_hours: updatedTransactionRow.hotel_rate_id ? (await client.query('SELECT hours FROM hotel_rates WHERE id = $1', [updatedTransactionRow.hotel_rate_id])).rows[0]?.hours : null,
      active_transaction_lifecycle_status: NEW_TRANSACTION_LIFECYCLE_STATUS,
      cleaning_status: Number(updatedRoomDbRow.cleaning_status),
      hotel_rate_id: updatedRoomDbRow.hotel_rate_id ? JSON.parse(updatedRoomDbRow.hotel_rate_id) : [],
    };
    ...
    return { success: false, message: `Database error during room assignment and check-in: ${error instanceof Error ? error.message : String(error)}` };
  } finally {
    if (client) {
      client.release();
