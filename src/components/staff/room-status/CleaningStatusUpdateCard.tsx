...
import CleaningNotesModal from './CleaningNotesModal';
...
interface CleaningStatusUpdateCardProps {
  allRoomsForBranch: HotelRoom[]; // Pass all rooms, filtering will happen here
  staffUserId: number | null;
  tenantId: number | null;
  branchId: number | null;
  onStatusUpdateSuccess: (updatedRoomData: Pick<HotelRoom, 'id' | 'cleaning_status' | 'cleaning_notes'>) => void;
  onRefreshDataNeeded: () => void; // Callback to tell parent to refetch all room data
  isLoadingParent: boolean; // Loading state from parent
}
...
const getDefaultNoteForModal = (status: number, currentNotes?: string | null): string => {
  ...
  if (status === ROOM_CLEANING_STATUS.OUT_OF_ORDER) {
      return ""; // Always start fresh for Out of Order, or use currentNotes if editing existing Out of Order
  }
  return currentNotes || "";
};
...
export default function CleaningStatusUpdateCard({
  ...
  const handleSaveCleaningUpdateAndNotes = async (data: RoomCleaningStatusUpdateData) => {
    ...
    try {
      const result = await serverUpdateRoomCleaningStatusAction(
        ...
        tenantId,
        branchId,
        data.cleaning_status, // Send number
        data.cleaning_notes,
        staffUserId
      );
  ...
  const roomCountsByCleaningStatus = useMemo(() => {
    return allRoomsForBranch.reduce((acc, room) => {
      if (String(room.status) === HOTEL_ENTITY_STATUS.ACTIVE) { // Count only active room definitions
  ...
  const groupedFilteredRoomsForTab = useMemo(() => {
  ...
  const sortedFloorsForTab = useMemo(() => {
    return Object.keys(groupedFilteredRoomsForTab).sort((a, b) => {
      const numA = parseInt(a); const numB = parseInt(b);
      if (!isNaN(numA) && !isNaN(numB)) return numA - numB;
      if (!isNaN(numA)) return -1; if (!isNaN(numB)) return 1;
      return a.localeCompare(b);
    });
  }, [groupedFilteredRoomsForTab]);
        ...
        <div className="flex items-center justify-between">
            <div>
                ...
                <ShadCardDescription className="flex justify-between items-center">
                    <span>Quickly update the cleaning status for rooms. Click status icon to update notes and status.</span>
                </ShadCardDescription>
            </div>
