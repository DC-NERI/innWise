...
export async function archiveRate(rateId: number, tenantId: number, branchId: number, adminUserId: number): Promise<{ success: boolean; message?: string }> {
  ...
  try {
    ...
    `;
    const res = await client.query(query, [HOTEL_ENTITY_STATUS.ARCHIVED, rateId, tenantId, branchId]);

    if (res.rowCount > 0) {
      return { success: true, message: "Rate archived successfully." };
    }
    return { success: false, message: "Rate not found or archive failed." };
  } catch (error) {
    console.error('[archiveRate DB Error]', error);
    return { success: false, message: `Database error during rate archive: ${error instanceof Error ? error.message : String(error)}` };
  } finally {
    client.release();
  }
}
