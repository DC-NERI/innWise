  ...
  try {
    ...
    const transactionQuery = `
      INSERT INTO transactions (
        ...
        client_payment_method, notes, check_in_time, -- check_in_time here is when the reservation is made
      ...
      RETURNING id, client_name, check_in_time, hotel_rate_id, reserved_check_in_datetime, status, is_paid;
    `;
    ...
    const transactionValues = [
      ...
      finalPaymentStatus.toString(), // $10 is_paid
      ...
      TRANSACTION_IS_ACCEPTED_STATUS.ACCEPTED.toString(), // $14 is_accepted (staff creating for their branch implies acceptance)
    ...
    await client.query(roomUpdateQuery, [
      ROOM_AVAILABILITY_STATUS.RESERVED.toString(),
      newTransaction.id,
      roomId,
      tenantId,
      branchId
    ...
    // Fetch rate name for returning to client
    ...
    if (dbError.code === '23505' && dbError.constraint && dbError.constraint.includes('hotel_room_id_status_unique_active_reservations')) { // Hypothetical constraint name
